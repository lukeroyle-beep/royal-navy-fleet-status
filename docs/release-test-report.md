...
| Field | Value |
| --- | --- |
| Commit SHA | Deployed `main` at `18ed5da5a7124802545fdf11156d7e4731c5195d` |
...
| Orientation or viewport | iPhone portrait, 428 × 727 CSS-pixel viewport at 3× device scale; desktop window at normal zoom and 200% browser zoom |
...
| RT-3 | Blocked | Full touch map-control matrix incomplete | Physical iPhone taps opened, closed and reopened the Fleet drawer; pan, pinch and both map zoom buttons were not recorded. |
...
| RT-13 | Blocked | Full touch-target matrix incomplete | The physical iPhone Fleet control received the expected pointer, touch, mouse and click event sequence, with no overlay intercepting the hit target; adjacent controls were not fully exercised. |
...
This verifies the deployed cache-recovery fix against the originally affected physical iPhone. It
does not replace the remaining iPad, orientation, map-gesture or human assistive-technology checks.
...
| DEVICE-1 | Release-blocking | Required physical iPad, prescribed private HTTPS and complete iPhone/desktop scenarios were not performed. | Physical iPhone production loading, hit-testing and Fleet interaction are now verified. The iPad was only reported to load; its model, orientations and interaction results were not recorded. The local desktop pass was intentionally limited to non-destructive supporting checks. | Complete the remaining RT-1 to RT-18 checks on the required private device matrix. |
