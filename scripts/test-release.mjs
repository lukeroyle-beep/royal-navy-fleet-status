...
assert.doesNotMatch(workflow, /actions\/(?:checkout|setup-node|upload-artifact)@v4\b/);
...
assert.match(discoveryWorkflow, /actions\/checkout@[0-9a-f]{40}\s+# v4/);
assert.match(discoveryWorkflow, /actions\/setup-node@[0-9a-f]{40}\s+# v4/);
assert.match(discoveryWorkflow, /actions\/upload-artifact@[0-9a-f]{40}\s+# v4/);
...
assert.match(appSource, /createPublicationComparison\(insights\.changes, currentDataset\.vessels\)/);
assert.doesNotMatch(appSource, /compareCurrentWithPreviousSnapshot/);
